...
void UART_init(int baudrate) {
	...
	GPIO_Init(GPIOB, &(GPIO_InitTypeDef ) { .GPIO_Pin = GPIO_Pin_7, .GPIO_Mode =
					GPIO_Mode_AF, .GPIO_PuPd = GPIO_PuPd_UP });

	GPIO_PinAFConfig(GPIOB, GPIO_PinSource6, GPIO_AF_USART1);
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource7, GPIO_AF_USART1);

	USART_Init(OUT_USART, &(USART_InitTypeDef ) { .USART_BaudRate = baudrate,
